/*
 * File: \HuaWeiJiShi\HJ24_Chorus\chorus_test.go                               *
 * Project: newcoder                                                           *
 * Created At: Friday, 2022/04/22 , 14:25:47                                   *
 * Author: elchn                                                               *
 * -----                                                                       *
 * Last Modified: Friday, 2022/04/22 , 22:33:50                                *
 * Modified By: elchn                                                          *
 * -----                                                                       *
 * HISTORY:                                                                    *
 * Date      	By	Comments                                                   *
 * ----------	---	---------------------------------------------------------  *
 */

package main

import "testing"

func TestChorus(t *testing.T) {
	funcs := map[string]func(nums []uint) uint{
		// Chorus1,
		"Chorus2": Chorus2,
		"Chorus3": Chorus3,
		"Chorus4": Chorus4,
	}
	tests := []struct {
		heights []uint
		res     uint
	}{
		{
			[]uint{
				200,
			},
			0,
		},

		{
			[]uint{
				186, 186, 150, 200, 160, 130, 197, 200,
			},
			4,
		},
		{
			[]uint{
				// 186, 186, 150, 200, 160, 130, 197, 200,
				186, 186, 186, 156,
			},
			3,
		},
		{
			[]uint{
				// 186, 186, 150, 200, 160, 130, 197, 200,
				186, 186, 186,
			},
			2,
		},
		{
			[]uint{
				16, 103, 132, 23, 211, 75, 155, 82, 32, 48, 79, 183, 13, 91, 51, 172, 109, 102, 189, 121, 12, 120, 116, 133, 79, 120, 116, 208, 47, 110, 65, 187, 69, 143, 140, 173, 203, 35, 184, 49, 245, 50, 179, 63, 204, 34, 218, 11, 205, 100, 90, 19, 145, 203, 203, 215, 72, 108, 58, 198, 95, 116, 125, 235, 156, 133, 220, 236, 125, 29, 235, 170, 130, 165, 155, 54, 127, 128, 204, 62, 59, 226, 233, 245, 46, 3, 14, 108, 37, 94, 52, 97, 159, 190, 143, 67, 24, 204, 39, 222, 245, 233, 11, 80, 166, 39, 224, 12, 38, 13, 85, 21, 47, 25, 180, 219, 140, 201, 11, 42, 110, 209, 77, 136,
			},
			95,
		},
		{
			[]uint{
				16, 103, 132, 23, 211, 75, 155, 82, 32, 48, 79, 183, 13, 91, 51,
			},
			7,
		},
	}

	for fn, f := range funcs {
		for _, test := range tests {
			if res := f(test.heights); res != test.res {
				t.Errorf("%s(%v) = %d,want %d\n", fn, test.heights, res, test.res)
			}
		}
	}
}
